...
class LaneSpeed:
  ...
  def group_tracks(self):
    ...
    for track, y_rel_corrected in zip(self.live_tracks, track_y_rels_corrected):
      ...
      if self.lanes['left'].bounds[0] >= track.yRel - y_rel_corrected >= self.lanes['left'].bounds[1]:
        if track_vel >= 2.24:
          self.lanes['left'].tracks.append(track)
        elif track_vel <= -2.24:
          self.lanes['left'].oncoming_tracks.append(track)
      elif self.lanes['middle'].bounds[0] >= track.yRel - y_rel_corrected >= self.lanes['middle'].bounds[1]:
        if track_vel >= 2.24:
          self.lanes['middle'].tracks.append(track)
        elif track_vel <= -2.24:
          self.lanes['middle'].oncoming_tracks.append(track)
      elif self.lanes['right'].bounds[0] >= track.yRel - y_rel_corrected >= self.lanes['right'].bounds[1]:
        if track_vel >= 2.24:
